...
def generate():
    ...
    queryset_base = (Queryset("conflictology_cmbaseline", "country_month")
        ...
        # Apply logarithmic transformation, handle missing values by replacing them with NA
        .with_column(Column("ln_ns_best", from_loa="country_month", from_column="ged_ns_best_sum_nokgi")
            .transform.ops.ln().transform.missing.replace_na())
        
        # Create a new column 'ln_os_best' using data from 'priogrid_month' and 'ged_os_best_count_nokgi' column
        # Apply logarithmic transformation, handle missing values by replacing them with NA
        .with_column(Column("ln_os_best", from_loa="country_month", from_column="ged_os_best_sum_nokgi")
            .transform.ops.ln().transform.missing.replace_na())
