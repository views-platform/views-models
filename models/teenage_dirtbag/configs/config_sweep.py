def get_sweep_config():
    ...
    parameters = {
        # Temporal horizon & context - longer for dilated convolutions
        'steps': {'values': [[*range(1, 36 + 1)]]},
        'input_chunk_length': {'values': [48, 60, 72, 84]},  # Longer for dilations
        ...
        # TCN specific architecture - dilations for long-range patterns
        'kernel_size': {'values': [3, 5, 7]},  # Larger kernels for patterns
        'num_filters': {'values': [32, 64, 128]},  # More filters for complexity
        'dilation_base': {'values': [2, 3, 4]},  # Higher bases for longer context
        'dropout': {'values': [0.1, 0.2, 0.3]},
        'use_reversible_instance_norm': {'values': [True, False]},  # Test both
        'weight_norm': {'values': [True, False]},  # Test both
