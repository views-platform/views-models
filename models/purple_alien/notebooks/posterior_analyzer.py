# Re-posting the complete, polished PosteriorAnalyzer class and final test hook
...
class PosteriorAnalyzer:
    ...
    def test_posterior_analyzer(verbose: bool = True) -> Tuple[List[str], List[str]]:
        ...
        for name, samples in test_cases.items():
            try:
                analyzer = PosteriorAnalyzer(samples, credible_masses=(0.5, 0.95, 0.99))
                hdis = analyzer.summary['hdis']
                map_val = analyzer.summary['map']
