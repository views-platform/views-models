...
A LocfModel trained on synthetically generated data with a `vertical_stripe` spatial pattern. The model predicts `synth_target` at the priogrid-month (pgm) level across 1000 grid cells.
...
where `col(i) = (priogrid_gid - 1) mod 720`. This produces values in [0, 9] repeating every 10 columns. The pattern is **time-invariant** -- every month gets identical values.
