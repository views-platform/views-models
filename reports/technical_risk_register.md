...
**Concerns:** Open 46 | Mitigated 19 | Resolved 38 | Accepted 3 | Partially Resolved 1 | Subsumed 1  
...
| **Status** | Mitigated (2026-07-20): the ADR-013 wire's per-run **manifest** is exactly the deterministic-addressing exit; type-guards now bound the legacy path. Residual until the wire is live end-to-end. |
...
| **Status** | **Subsumed by þing-01 (PLATFORM-001), 2026-07-28** — the secrets-management architecture is decided by the ratified cross-repo verdict; the risk is now owned by that contract + its per-repo follow-through, not by this entry (was: Open, architecture deferred). |
