...
import os
from views_pipeline_core.templates.package import (template_example_manager, template_gitignore)
...
class PackageScaffoldBuilder:
    ...
    def build_package_scripts(self):
        template_example_manager.generate(self._package_manager.manager / "example_manager.py")

if __name__ == "__main__":

    package_name = str(input(f"Enter the name of the package: "))
    while (
        PackageManager.validate_package_name(package_name) == False
    ):
    ...
    package_path = str(input(f"Using the package path '{Path(os.getcwd()) / package_name}'. Press Enter to confirm or enter a new base directory: "))
    if package_path == "":
    ...
    package_scaffold_builder = PackageScaffoldBuilder(PackageManager(package_path, validate=False))
